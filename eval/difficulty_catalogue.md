...
| 37 | `refuse_onset_clipped` **[M]** | dict(`Aaditya`; `aaditya, aditya`) | `ditya will confirm` | `Ditya will confirm.` | unchanged | ABSTAIN | ABSTAIN | D12 + **W4** first letter. `ditya` and `aaditya` do share metaphone `TTY`, so the phonetic retriever would have matched — only the retriever's first-letter rule stops it. (W2 is separately true: `ditya`→`Aaditya` is also unlearnable by correction.) Uses the brief entity only as *setup*, asserts nothing about it |
